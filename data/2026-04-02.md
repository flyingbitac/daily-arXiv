...
TL;DR: VLLR提出了一种结合LLM/VLM外部奖励和策略自我确定性内在奖励的密集奖励框架，用于微调机器人基础策略，无需手动奖励工程，在CHORES基准上显著提升任务成功率。
...
Method: VLLR框架结合：(1) 使用LLM分解任务为可验证子任务，VLM估计进度作为外部奖励；(2) 基于策略自我确定性的内在奖励。通过VLM初始化价值函数进行短暂预热，避免训练期间的高推理成本，自我确定性在PPO微调中提供每步内在指导。

Result: 在CHORES基准测试中，VLLR相比预训练策略获得高达56%的绝对成功率提升，相比最先进的RL微调方法在分布内任务上提升5%，在分布外任务上提升10%。消融研究表明VLM价值初始化主要提升任务完成效率，自我确定性主要提高成功率。

Conclusion: VLLR通过结合LLM/VLM的外部奖励和策略自我确定性的内在奖励，实现了无需手动奖励工程的机器人策略微调，显著提升了长时程任务的性能，特别是在分布外任务上的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统路径规划器主要优化几何成本，对人类操作员明确指定路径的支持有限，而机器人在人机共享环境中需要反映人类空间意图的路径

Method: 开发MRReP混合现实界面，用户通过手势在物理地面上绘制手绘参考路径(HRP)，通过自定义的HRP规划器将用户指定的点序列转换为全局路径用于自主导航
...
TL;DR: 提出分层规划与控制框架，结合长时域引导和几何感知安全保证，用于多面体机器人在多面体障碍物环境中的自主导航。


<details>
  <summary>Details</summary>
Motivation: 复杂非凸环境中考虑机器人动力学、控制限制和精确几何形状的自主导航仍然具有挑战性，需要同时解决长时域规划和几何安全约束问题。

Method: 分层框架：高层使用MILP嵌入MPC生成名义轨迹（将机器人简化为质点）；低层使用基于闵可夫斯基差空间精确符号距离的CBF作为安全滤波器，并扩展到高阶CBF，显式执行机器人几何约束。

Result: 在U形和迷宫环境中测试单积分器和双积分器动力学，结果表明该架构缓解了纯反应式CBF导航的拓扑诱导局部极小行为，实现了安全、实时、几何感知的导航。

Conclusion: 提出的分层框架成功结合了长时域规划和几何安全保证，在复杂环境中实现了既考虑机器人几何形状又保持计算可行性的自主导航。
...
TL;DR: 本文提出了一种改进的框架，通过直接在仿人机器人运动空间训练引导扩散模型，聚合多样的人类和机器人数据集到统一的体现空间，从而获得更广泛的技能覆盖和更自动化的流程。


<details>
  <summary>Details</summary>
Motivation: 开发稳健的仿人机器人自主移动操作技能仍然是一个开放问题。虽然强化学习在腿部运动方面应用成功，但将其应用于复杂、交互丰富的操作任务更加困难，因为需要长时程规划。现有的DreamControl方法利用现成的人类运动扩散模型作为生成先验来指导强化学习策略训练，但存在局限性。

Method: 提出改进框架：1）直接在仿人机器人运动空间训练引导扩散模型；2）聚合多样的人类和机器人数据集到统一的体现空间；3）扩展参考轨迹生成以提高下游强化学习策略的稳健性。

Result: 该方法能够捕获更广泛的技能（得益于更大的训练数据混合），建立了更自动化的流程（无需手动过滤干预），并通过扩展参考轨迹生成实现了更稳健的下游强化学习策略。在仿真和真实Unitree-G1机器人上进行了广泛实验验证。

Conclusion: 直接在机器人运动空间训练扩散模型并聚合多源数据到统一体现空间的方法，相比现有技术能够更好地支持仿人机器人的自主移动操作技能学习，提供了更自动化、更稳健的解决方案。
...
TL;DR: REMind是一个创新的教育机器人角色扮演游戏，旨在支持儿童反欺凌旁观者干预，通过社交机器人演绎欺凌场景，让玩家观察、反思角色视角，并通过操控机器人化身练习防御策略。


<details>
  <summary>Details</summary>
Motivation: 设计一个创新的教育工具来支持儿童反欺凌旁观者干预，利用机器人技术增强社会情感学习效果。

Method: 开发了REMind系统，让玩家观察社交机器人演绎的欺凌场景，反思角色视角，并通过操控机器人化身练习防御策略。采用混合方法对18名9-10岁儿童进行游戏测试研究。

Result: 研究发现该体验支持了关键学习目标，包括自我效能感、换位思考、理解防御结果和干预策略。这些结果凸显了机器人媒介应用戏剧作为支持社会情感学习的新教学框架的潜力。

Conclusion: REMind展示了机器人媒介应用戏剧作为支持社会情感学习的新教学框架的潜力，为儿童反欺凌旁观者干预提供了有效的教育工具。
...
TL;DR: 提出分层规划控制框架，在未知非线性动力学下进行在线运动规划，通过自适应状态空间分区和增量图搜索实现探索与利用的平衡


<details>
  <summary>Details</summary>
Motivation: 在未知非线性动力学下进行自主运动规划需要同时学习系统特性并导航到目标，现有方法往往需要先验系统知识或计算复杂度高

Method: 1) 将状态空间划分为多面体，用分段仿射模型近似未知非线性系统；2) 引入非均匀自适应状态空间分区策略，仅在任务相关区域细化分区；3) 将PWA系统抽象为有向加权图，边存在性通过可达控制理论和预测可达条件增量验证；4) 对已认证边使用可证明的到达时间界限加权，对不确定边使用信息论权重引导探索；5) 引入松弛可达条件扩展框架到欠驱动系统

Result: 仿真结果表明该方法能有效平衡探索与利用，同时提供形式化的可达性保证

Conclusion: 该分层规划控制框架能够在有限先验系统知识下实现在线运动合成，通过自适应分区和图抽象方法降低了计算复杂度，并扩展到欠驱动系统
...
TL;DR: 该研究提出了一种让空中机器人能够使用机载传感器和嵌入式飞行硬件实时推理周围风场的方法，核心创新是将地形测量数据与稀疏现场风测量数据融合来预测周围流场。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决两个基本问题：1）地形数据是否足以在密集城市环境中准确预测风场；2）学习到的风模型在运动规划中的实用性，特别是对能源效率和避障的影响。通过让机器人能够快速评估局部风况而无需先验环境知识，加速空中机器人进入更具挑战性的环境。

Method: 结合深度学习、流体力学和最优控制工具，建立使用导航LiDAR进行局部风预测的框架，然后将局部风模型先验纳入后退时域最优控制器中，研究局部风知识如何影响自主导航中的能源使用和鲁棒性。

Result: 通过在不同城市风场景中的模拟演示评估了风预测器的预测能力，并量化了将局部风信息集成到运动规划中对自主城市导航的改进（碰撞率和能耗）。亚尺度自由飞行实验表明这些算法可以在嵌入式飞行计算机上实时运行，具有足够带宽来稳定控制小型空中机器人。

Conclusion: 该论文为未知风环境中的局部风推理和运动规划贡献了新的范式，使机器人能够无需先验环境知识快速评估局部风况，加速了空中机器人进入更具挑战性环境的进程。
...
TL;DR: 使用Webots模拟平台研究Braitenberg机器人的群体捕食者躲避和围攻行为，探讨围攻呼叫范围和群体规模对围攻成功率的影响。


<details>
  <summary>Details</summary>
Motivation: 模仿动物群体通过围攻呼叫协调攻击捕食者的行为机制，研究人工生命中的行动选择模拟和自主智能体控制架构设计。

Method: 使用Webots机器人仿真平台模拟Braitenberg机器人的群体行为。机器人将光源视为捕食者，当面对光源时发出围攻呼叫召集同伴，如果成功召集盟友则围攻光源，否则逃离。实验变量包括围攻呼叫范围（无限、中、低范围）和群体规模（10个vs3个机器人）。

Result: 研究结果表明，围攻呼叫范围和群体规模都对围攻成功率有显著影响。这两个变量在群体围攻行为的成功协调中起着重要作用。

Conclusion: 这项工作对人工生命中的行动选择模拟和自主智能体控制架构设计具有重要意义，展示了群体协调机制在机器人行为控制中的应用潜力。
...
TL;DR: 本文提出了一种热红外与深度（TIR-D）融合的鲁棒人员跟踪架构，用于解决RGB-D跟踪在极端光照条件下的性能退化问题，通过序列知识转移策略和细粒度差分学习率策略，在SLAM机器人标准传感器套件上实现了全天候鲁棒跟踪。


<details>
  <summary>Details</summary>
Motivation: RGB-D跟踪在完全黑暗或强烈背光等极端光照条件下性能严重下降，而自主移动机器人需要在多样化和不可预测的环境中实现鲁棒的人员跟踪。热红外传感器在恶劣光照条件下表现稳定，但缺乏标注的多模态数据集限制了TIR-D跟踪的发展。

Method: 提出TIR-D跟踪架构，利用SLAM机器人的标准传感器套件（LiDAR和热红外相机）。采用序列知识转移策略，从大规模热红外预训练模型中演化结构先验到TIR-D领域。使用细粒度差分学习率策略，在保持预训练特征提取能力的同时，快速适应几何深度线索。

Result: 实验结果表明，提出的TIR-D跟踪器在平均重叠度（AO）上达到0.700，成功率（SR）达到58.7%，显著优于传统的RGB迁移方法和单模态基线方法。

Conclusion: 该方法为全天候机器人应用中鲁棒的人员跟随提供了实用且资源高效的解决方案，通过有效融合热红外和深度信息，克服了极端光照条件下的跟踪挑战。
...
TL;DR: 该论文提出了一种在不确定环境中集成任务和动力学运动规划的方法，使用部分可观测随机混合系统建模，并开发了结合决策技术和采样运动规划的随时算法。


<details>
  <summary>Details</summary>
Motivation: 解决在不确定环境中机器人同时执行任务规划和动力学运动规划的挑战，特别是环境语义标签的不确定性，以及非线性动力学系统与有限迹线性时序逻辑规范的集成问题。

Method: 将问题建模为部分可观测随机混合系统，捕获机器人动力学、LTLf任务和环境状态变量不确定性。提出一种随时算法，利用混合系统结构，结合决策技术和基于采样的运动规划方法。

Result: 证明了算法的正确性和渐近最优性。实验结果表明算法在不确定环境中有效，且始终优于基线方法。

Conclusion: 提出的方法成功解决了不确定环境中的集成任务和动力学运动规划问题，通过混合系统建模和结合决策与运动规划的算法，实现了高效且最优的解决方案。
...
### [11] [Certificate-Driven Closed-Loop Multi-Agent Path Finding with Inheritable Factorization](https://arxiv.org/abs/2604.00428)
*Jiarui Li,Runyu Zhang,Gioele Zardini*

Main category: cs.RO

TL;DR: CDCBS通过引入证书轨迹和舰队预算机制改进ACCBS，在有限计算预算下提供更一致的解质量，特别是在密集场景中。


<details>
  <summary>Details</summary>
Motivation: 现有闭环MAPF算法（如ACCBS）在有限视野规划下可能短视，难以保持全局保证和利用组合结构，特别是在密集实例中可能以短活动前缀终止。

Method: 引入证书轨迹及其关联的舰队预算作为过滤闭环更新的通用机制。证书提供无冲突回退计划和剩余成本的单调上界；仅接受证书改进的更新保证完备性。预算信息诱导预算限制分解，实现跨时间步的全局可继承分解。

Result: 在基准地图上的实验表明，CDCBS比ACCBS获得更一致的解质量，特别是在密集设置中，同时提出的分解减少了有效组大小。

Conclusion: 证书轨迹和舰队预算机制为闭环MAPF算法提供了有效的全局保证和分解能力，CDCBS在密集场景中表现出更好的性能一致性。
...
### [12] [Reachability-Aware Time Scaling for Path Tracking](https://arxiv.org/abs/2604.00439)
*Hossein Gholampour,Logan E. Beaver*

Main category: cs.RO

TL;DR: 提出一种用于平面双积分系统的路径跟踪方法，通过离线速度缩放和在线二次规划跟踪器，确保在有界加速度下跟踪采样规划器生成的避障路径


<details>
  <summary>Details</summary>
Motivation: 采样规划器生成的避障路径常包含急转弯和高曲率区域，即使在路径几何无碰撞的情况下，加速度限制下的单步可达性也变得至关重要

Method: 基于纯追踪式、可达性引导的二次规划跟踪器，具有单步加速度裕度。离线时沿样条路径评估该裕度并更新标量速度缩放剖面；在线时使用相同的前瞻跟踪结构跟踪缩放后的参考路径

Result: 通过速度缩放确保所需单步加速度始终低于可用边界，从而在有界速度和加速度约束下实现避障路径的安全跟踪

Conclusion: 该方法结合离线速度剖面优化和在线二次规划跟踪，有效解决了采样规划器生成路径在加速度约束下的跟踪问题，提高了系统在复杂环境中的运动安全性
...
### [13] [Multi-Camera View Scaling for Data-Efficient Robot Imitation Learning](https://arxiv.org/abs/2604.00557)
*Yichen Xie,Yixiao Wang,Shuqi Zhao,Cheng-En Wu,Masayoshi Tomizuka,Jianwen Xie,Hao-Shu Fang*

Main category: cs.RO

TL;DR: 通过多视角相机采集专家演示数据，无需额外人工成本即可丰富训练分布，提升模仿学习策略的泛化能力


<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习的泛化能力受限于专家演示的多样性，而收集多样化环境的演示成本高且困难。需要一种实用方法在不增加人工成本的情况下利用场景多样性。

Method: 提出多视角框架：在演示采集时使用多个同步相机视角，从每个专家轨迹生成伪演示；分析不同动作空间与视角扩展的交互；引入多视角动作聚合方法，使单视角策略在部署时也能受益于多相机。

Result: 在仿真和真实世界操作任务中的大量实验表明，相比单视角基线，该方法在数据效率和泛化能力方面都有显著提升。
...
### [14] [A Physical Imitation Learning Pipeline for Energy-Efficient Quadruped Locomotion Assisted by Parallel Elastic Joint](https://arxiv.org/abs/2604.00611)
*Huyue Ma,Yurui Jin,Helmut Hauser,Rui Wu*

Main category: cs.RO

TL;DR: 该论文提出物理模仿学习(PIL)，将强化学习控制策略蒸馏到可物理实现的被动并联弹性关节中，让机器人身体模仿部分控制行为，从而降低能耗


<details>
  <summary>Details</summary>
Motivation: 动物通过脑体协同进化，利用内在身体动力学实现节能运动，而机器人通常采用集中式控制器抑制而非利用身体动力学。需要开发能够利用身体被动特性的控制方法

Method: 提出物理模仿学习(PIL)：1) 将强化学习控制策略蒸馏到可物理实现的被动并联弹性关节(PEJs)中；2) 剩余策略控制电机恢复RL策略性能；3) 通过将部分控制策略卸载到PEJs来降低能耗

Result: 在模拟四足机器人中，PIL方法在平坦地形上可将高达87%的机械功率卸载到PEJs，在崎岖地形上可卸载18%。实现了脑体协同设计而不扩展搜索空间

Conclusion: PIL提供了一种计算高效的方法来实现任务特定的体现物理智能，适用于广泛的关节型机器人形态，通过将控制策略蒸馏到身体设计中而非联合优化，实现了脑体协同设计
...
### [15] [StretchBot: A Neuro-Symbolic Framework for Adaptive Guidance with Assistive Robots](https://arxiv.org/abs/2604.00628)
...
TL;DR: StretchBot：结合多模态感知与知识图谱的大型语言模型机器人教练，用于自适应辅助拉伸指导，相比脚本化系统能更好地适应用户状态和环境上下文。


<details>
  <summary>Details</summary>
Motivation: 辅助机器人在家庭和医疗场景中支持身体健康的潜力日益增长，但现有系统大多为脚本化，限制了其适应用户状态、环境上下文和交互动态的能力。

Method: 提出StretchBot，一种混合神经符号机器人教练，结合多模态感知与基于知识图谱的大型语言模型推理，支持在保持结构化常规的同时进行上下文感知调整。

Result: 探索性试点比较显示，自适应条件在感知适应性和上下文相关性方面有所改善，而脚本化指导在流畅性和可预测性方面仍具竞争力。

Conclusion: 结构化可操作知识可以帮助在具身辅助交互中基于语言模型进行适应，但需要更大规模的纵向研究来评估鲁棒性、泛化能力和长期用户体验。
...
### [16] [LiPS: Lightweight Panoptic Segmentation for Resource-Constrained Robotics](https://arxiv.org/abs/2604.00634)
*Calvin Galagain,Martyna Poreba,François Goulette,Cyrill Stachniss*

Main category: cs.RO

TL;DR: LiPS是一种轻量级全景分割方法，在保持基于查询的解码同时引入简化的特征提取和融合路径，显著降低计算需求，适合资源受限的移动机器人平台。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的全景分割模型复杂度高，不适合部署在资源受限的移动机器人平台上，需要一种高效计算的全景分割方法。

Method: 提出LiPS方法，保留基于查询的解码机制，同时引入简化的特征提取和融合路径，实现轻量级设计。

Result: 在标准基准测试中，LiPS达到与更重基线相当的精度，同时提供高达4.5倍的吞吐量（帧率），计算需求减少近6.8倍。

Conclusion: LiPS的高效性使其成为现代全景分割模型与现实世界机器人应用之间的重要桥梁，适合资源受限的移动机器人平台。
...
### [17] [SoftHand Model-W: A 3D-Printed, Anthropomorphic, Underactuated Robot Hand with Integrated Wrist and Carpal Tunnel](https://arxiv.org/abs/2604.00738)
*Dhillon B. Merritt,Christopher J. Ford,Haoran Li,Malia Smith,Zhixing Chen,Efi Psomopoulou,Nathan F. Lepora*

Main category: cs.RO

TL;DR: 本文介绍了SoftHand Model-W：一款基于Pisa/IIT SoftHand的3D打印欠驱动仿人机器人手，集成了拮抗肌腱机制和2自由度肌腱驱动腕部，通过四个驱动自由度实现手指主动屈伸和手掌腕部主动屈伸/桡尺偏转，同时保持SoftHand的协同和自适应特性。


<details>
  <summary>Details</summary>
Motivation: 开发一种具有腕部驱动功能的仿人机器人手，以减少机器人手臂的补偿性和重新配置运动，提高操作效率和灵活性，实现更接近人类的灵巧操作。

Method: 采用3D打印技术制造欠驱动仿人机器人手，集成拮抗肌腱机制和2自由度肌腱驱动腕部；通过腕管式肌腱布线实现远程电机放置；将SoftHand-W安装在6轴机器人手臂上，进行立方体堆叠和平面圆盘旋转两种重新定向任务的测试。

Result: 与无腕部驱动的情况相比，添加腕部驱动显著减少了手臂的补偿性和重新配置运动，缩短了任务完成时间；腕部使原本不可能的拾取放置操作成为可能；腕部驱动对于实现多功能、类人操作至关重要。

Conclusion: SoftHand Model-W展示了近端自由度对于在现实世界机器人应用中实现多功能、类人操作的关键作用，其紧凑设计使其适用于研究和辅助场景。
...
### [18] [How to Train your Tactile Model: Tactile Perception with Multi-fingered Robot Hands](https://arxiv.org/abs/2604.00744)
*Christopher J. Ford,Kaichen Shi,Laura Butcher,Nathan F. Lepora,Efi Psomopoulou*

Main category: cs.RO

TL;DR: TacViT：基于视觉Transformer的新型触觉感知模型，能够在未见过的传感器上实现泛化，减少数据收集和重新训练需求


<details>
  <summary>Details</summary>
Motivation: 当前基于CNN的触觉感知方法需要大量传感器特定数据集，且每次部署新传感器都需要重新训练，这限制了触觉传感器在机器人操作中的快速部署和可扩展性

Method: 提出TacViT模型，基于视觉Transformer架构，利用全局自注意力机制从触觉图像中提取鲁棒特征，实现对接触属性的准确推断

Result: 在五指机器人手的传感器上评估TacViT，相比CNN表现出更优越的泛化性能，能够准确推断未见传感器的接触属性

Conclusion: TacViT有潜力使触觉感知更具可扩展性和实用性，加速新传感器的部署，推动触觉传感在真实世界机器人应用中的发展
...
### [19] [A wearable haptic device for edge and surface simulation](https://arxiv.org/abs/2604.00752)
*Rui Chen,Xianlong Mai,Alireza Sanaei,Domenico Chiaradia,Antonio Frisoli,Daniele Leonardis*

Main category: cs.RO

TL;DR: 提出了一种轻量级指尖触觉设备，通过双电机机制提供可区分的表面和边缘接触反馈，在VR环境中增强物体操纵的真实感。


<details>
  <summary>Details</summary>
Motivation: 传统指尖触觉设备无法渲染某些对沉浸式和精确交互至关重要的触觉特征（如边缘检测），这限制了VR应用中物体操纵的真实感和精确性。

Method: 开发了一种紧凑轻量（24.3克）的指尖触觉设备，采用新颖的双电机机制，通过6×6柔性传感器阵列进行压力分布表征，并通过用户研究评估设备性能。

Result: 压力分布表征显示两种刺激模式具有不同的接触模式；初步用户研究（5名参与者）在四种条件下（边缘/表面接触×轻/重压力）平均分类准确率达到93%，平均响应时间为2.79秒。

Conclusion: 该设备能够有效传递边缘和表面触觉线索，有望提升VR环境中物体操纵的保真度，为更沉浸的虚拟交互体验提供技术支持。
...
### [20] [A Dual-Action Fabric-Based Soft Robotic Glove for Ergonomic Hand Rehabilitation](https://arxiv.org/abs/2604.00768)
...
TL;DR: 研究人员开发了一种基于织物的双动作软体机器人手套，用于手部康复和辅助，该手套具有定制化设计、更好的贴合度和同时支持手指屈伸功能。


<details>
  <summary>Details</summary>
Motivation: 神经系统疾病导致的手部功能障碍严重影响日常生活独立性，现有软体机器人手套在定制化、贴合度和屈伸驱动方面存在局限，限制了临床实用性。

Method: 采用计算机数控热封技术制造对称腔室执行器，设计具有五个独立控制的双动作执行器（支持手指屈伸）和专门的拇指外展执行器，执行器充气时形成凹形外表面以最大化手指接触面积。

Result: 执行器产生足够的关节力矩和指尖力用于日常生活任务，手套系统能提供足够的抓握力；健康受试者测试显示前臂肌肉活动显著减少；脊髓损伤患者测试表明促进更自然的抓握模式，减少对肌腱转移抓握的依赖，但任务完成时间增加。

Conclusion: 这种可定制、符合人体工程学的设计代表了向个性化手部康复和辅助机器人技术迈出的实际一步，尽管当前驱动接口导致任务完成时间增加。
...
### [21] [Compact Keyframe-Optimized Multi-Agent Gaussian Splatting SLAM](https://arxiv.org/abs/2604.00804)
*Monica M. Q. Li,Pierre-Yves Lajoie,Jialiang Liu,Giovanni Beltrame*

Main category: cs.RO

TL;DR: 提出改进的多智能体RGB-D高斯溅射SLAM框架，通过压缩冗余高斯和两种闭环检测模式，在保持地图质量的同时减少85-95%的通信负载


<details>
  <summary>Details</summary>
Motivation: 多智能体3D建图中，密集表示（如高斯溅射）在有限带宽下实时交换困难，现有集中式SLAM系统在合并本地地图时通信成为瓶颈

Method: 1) 在SLAM系统中加入压缩步骤去除冗余3D高斯而不降低渲染质量；2) 采用无需初始猜测的集中式闭环检测，提供两种模式：纯渲染深度模式（仅需3D高斯）和相机深度模式（包含轻量深度图像以提升配准精度和进一步高斯剪枝）

Result: 在合成和真实数据集上的评估显示，相比最先进方法，两种模式下传输数据减少85-95%，使3D高斯多智能体SLAM更接近实际部署

Conclusion: 提出的改进框架显著降低了多智能体高斯溅射SLAM的通信负载，同时保持地图保真度，为实际场景部署提供了可行方案
...
### [22] [PanoAir: A Panoramic Visual-Inertial SLAM with Cross-Time Real-World UAV Dataset](https://arxiv.org/abs/2604.00852)
*Yiyang Wu,Xiaohu Zhang,Yanjin Du,Tongsu Zhang,Chujun Li,Siyang Chen,Guoyi Zhang,Xiangpeng Xu*

Main category: cs.RO

TL;DR: 本文针对无人机应用中的姿态估计问题，提出了一个全景视觉惯性SLAM框架，并构建了首个真实世界的全景视觉惯性数据集，以解决传统方法视野受限导致的漂移和失效问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉惯性SLAM方法主要依赖有限视野的传感器，在复杂的无人机场景中容易导致漂移甚至失效。虽然全景相机提供了全向感知能力，但全景视觉惯性SLAM及其对应的真实世界无人机数据集仍未被充分探索。

Method: 首先构建了一个覆盖多样化飞行条件的真实世界全景视觉惯性数据集。然后提出了一个全景视觉惯性SLAM框架，通过全景特征提取和全景闭环检测来利用全向视野，增强特征约束并确保全局一致性。

Result: 在提出的数据集和公共基准测试上的广泛实验表明，该方法相比现有方法在准确性、鲁棒性和一致性方面表现更优。在嵌入式平台上的部署验证了其实用性，实现了与PC实现相当的计算效率。

Conclusion: 该研究通过构建首个真实世界全景视觉惯性数据集并开发相应的SLAM框架，有效解决了无人机应用中传统视觉惯性SLAM方法视野受限的问题，提高了姿态估计的准确性和鲁棒性。
...
### [23] [An Integrated Soft Robotic System for Measuring Vital Signs in Search and Rescue Environments](https://arxiv.org/abs/2604.00971)
*Jorge Francisco García-Samartín,Christyan Cruz Ulloa,Andrés Sánchez-Silva,Jaime del Cerro,Antonio Barrientos*

Main category: cs.RO

TL;DR: 开发了一种用于灾后救援的软体夹爪机器人系统，能够测量受害者的脉搏和血压，在灾后环境中实现快速、安全的生命体征监测。


<details>
  <summary>Details</summary>
Motivation: 虽然机器人已在搜救行动中得到广泛应用，受害者的评估技术也取得了显著进展，但在心率测量方面仍存在未解决的问题，特别是在灾后场景中缺乏血压评估的研究。

Method: 设计了一个软体夹爪并集成到移动机器人系统中，夹爪能够包裹受害者手臂并像血压计一样充气，通过专门的便携系统实现。采用不同的信号处理算法来处理测量数据。

Result: 系统实现了脉搏偏差4次/分钟，收缩压和舒张压偏差约5毫米汞柱的测量精度。误差项的同方差性验证和其他统计数据证明了系统能够准确确定心率和血压。

Conclusion: 该系统适合用于搜救行动，通过灾后场景测试验证了其功能，展示了其适应不同受害者位置的能力、测量速度以及对受害者的安全性。
...
### [24] [Infinite-Horizon Ergodic Control via Kernel Mean Embeddings](https://arxiv.org/abs/2604.01023)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于核的遍历控制方法虽然在一般覆盖域上提供了强大的覆盖保证，但由于计算复杂度随时间的指数增长，实际应用仅限于有限时域，无法用于长期覆盖任务。

Method: 推导了一种无限时域遍历控制器，配备扩展的核均值嵌入误差访问状态，该状态递归记录状态访问历史；同时提出了基于扩展误差状态的滚动时域控制变体。

Result: 证明了所推导控制器的渐近收敛性，并在2D和3D覆盖问题中展示了保持遍历覆盖保证的能力，解决了长期覆盖的计算可扩展性问题。

Conclusion: 通过引入扩展的误差访问状态，成功将遍历控制扩展到无限时域设置，为长期覆盖任务提供了计算可行的解决方案，同时保持了理论上的覆盖保证。
...
### [25] [Stein Variational Uncertainty-Adaptive Model Predictive Control](https://arxiv.org/abs/2604.01034)
*Hrishikesh Sathyanarayan,Ian Abraham*

Main category: cs.RO

TL;DR: 提出一种基于Stein变分分布的鲁棒控制器，用于处理具有潜在参数不确定性的非线性动力系统，通过任务依赖的不确定性分布粒子近似，在保持计算并行性的同时实现鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统分布鲁棒优化(DRO)采用保守的最坏情况模糊集优化，可能牺牲名义性能。需要一种方法既能处理参数不确定性，又能避免过度保守的设计，专注于对闭环性能影响最大的参数敏感性。

Method: 将最优控制与Stein变分推断耦合，使用确定性粒子基近似任务依赖的不确定性分布，避免对不确定性模型的限制性参数假设，同时保持计算并行性。

Result: 在代表性控制问题上，相比名义控制器、集成方法和经典分布鲁棒基线，该方法实现了更好的性能-鲁棒性权衡。

Conclusion: 该方法在单一决策理论框架中协调了鲁棒控制和变分推断，适用于具有参数不确定性的广泛控制系统类别，通过围绕对任务目标最关键的相关不确定性塑造控制律来实现鲁棒性。
...
### [26] [BAT: Balancing Agility and Stability via Online Policy Switching for Long-Horizon Whole-Body Humanoid Control](https://arxiv.org/abs/2604.01064)
*Donghoon Baek,Sang-Hun Kim,Sehoon Ha*

Main category: cs.RO

TL;DR: BAT是一个在线策略切换框架，通过动态选择两个互补的全身RL控制器，在敏捷性和稳定性之间取得平衡，实现长时域全身行为控制。
...
Method: 提出BAT框架：1) 通过分层强化学习学习的切换策略，带有滑动时域策略预评估的专家指导；2) 选项感知的VQ-VAE，从离散运动token序列预测选项偏好；3) 通过置信度加权融合两个模块做出最终决策。

Result: 在Unitree G1人形机器人上的大量仿真和真实世界实验表明，BAT能够实现多功能的长时域运动操作任务，并在多样化任务中优于现有方法。

Conclusion: BAT框架通过动态策略切换成功解决了敏捷性、鲁棒性和精度之间的权衡问题，为复杂长时域全身行为控制提供了有效解决方案。
...
### [27] [VRUD: A Drone Dataset for Complex Vehicle-VRU Interactions within Mixed Traffic](https://arxiv.org/abs/2604.01134)
*Ziyu Wang,Hongrui Kou,Cheng Wang,Ruochen Li,Hubert P. H. Shum,Amir Atapour-Abarghouei,Yuxin Zhang*

Main category: cs.RO

TL;DR: 提出VRUD数据集，针对城市L4自动驾驶在复杂混合交通环境中的挑战，特别是高密度弱势道路使用者的不可预测行为，填补现有数据集在非结构化城市环境中的空白。


<details>
  <summary>Details</summary>
Motivation: 城市L4自动驾驶（特别是自动驾驶出租车）在复杂城市混合交通环境中面临严峻挑战，主要源于弱势道路使用者（VRU）的高密度和高度不确定的交互行为。现有开源数据集主要关注高速公路或规范交叉口等结构化场景，缺乏反映混乱、非结构化城市环境的数据。

Method: 提出高效、高精度的无人机数据集构建方法，建立车辆-弱势道路使用者交互数据集（VRUD）。数据采集自深圳典型的"城中村"区域，具有交通监管松散和极端遮挡的特点。数据集包含4小时4K/30Hz录像，涵盖11,479个VRU轨迹和1,939个车辆轨迹。基于新型向量碰撞时间（VTTC）阈值提取了4,002个多智能体交互场景，并配有标准OpenDRIVE高清地图。

Result: VRUD数据集的特点是VRU占所有交通参与者的约87%，显著超过现有基准。数据集提供了罕见的边缘案例资源，可用于提升自动驾驶系统在复杂非结构化城市环境中的安全性能。数据集已开源。

Conclusion: VRUD数据集填补了现有数据集在非结构化城市环境中的空白，为提升自动驾驶系统在复杂城市混合交通环境中的安全性能提供了有价值的资源。数据集的开源将促进相关研究的进一步发展。
...
### [28] [Deep Reinforcement Learning for Robotic Manipulation under Distribution Shift with Bounded Extremum Seeking](https://arxiv.org/abs/2604.01142)
*Shaifalee Saxena,Rafael Fierro,Alexander Scheinker*

Main category: cs.RO

TL;DR: 论文提出了一种结合强化学习与有界极值搜索的混合控制器，用于提升机器人操作任务在分布外条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作中表现良好，但训练的策略在测试条件与训练分布不同时性能会下降。这在接触丰富的任务（如推动、抓取放置）中尤为明显，目标、接触条件或机器人动力学的变化都可能导致推理时系统处于分布外状态。

Method: 提出混合控制器：在标准条件下使用深度确定性策略梯度（DDPG）训练机器人推动和抓取放置任务策略，然后在部署时结合有界极值搜索（ES）。RL策略提供快速操作行为，而有界ES确保当操作条件偏离训练条件时，整体控制器对时间变化的鲁棒性。

Result: 在多种分布外设置下评估控制器，包括时变目标和空间变化的摩擦斑块条件。

Conclusion: 结合强化学习与有界极值搜索的混合方法能够提升机器人操作任务在分布外条件下的鲁棒性，RL提供快速响应，ES确保对条件变化的适应性。
...
### [29] [SMASH: Mastering Scalable Whole-Body Skills for Humanoid Ping-Pong with Egocentric Vision](https://arxiv.org/abs/2604.01158)
...
<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人乒乓球系统存在两个核心限制：1）依赖外部感知系统；2）无法实现敏捷的全身协调以执行精确任务。这些限制源于快速机器人运动下的低延迟鲁棒机载感知挑战，以及获取多样化任务对齐击球动作的困难。

Method: 开发了模块化系统，统一了可扩展的全身技能学习和机载自我中心感知。通过生成模型增强和多样化击球动作，利用可扩展的运动先验，实现紧密协调的全身控制而非解耦的上下半身行为。

Result: 实现了首个仅使用机载感知就能进行连续击球的人形机器人乒乓球系统。在高速度条件下展示了稳定精确的球交换，包括爆炸性全身扣杀和低蹲击球等多样化击球动作。

Conclusion: 验证了可扩展、感知驱动的全身技能学习在动态人形交互任务中的有效性，为无需外部相机的人形机器人敏捷乒乓球交互提供了可行的解决方案。
...
### [30] [Collaborative Task and Path Planning for Heterogeneous Robotic Teams using Multi-Agent PPO](https://arxiv.org/abs/2604.01213)
*Matthias Rubio,Julia Richter,Hendrik Kolvenbach,Marco Hutter*

Main category: cs.RO

TL;DR: 本文提出基于MAPPO的多机器人协同规划策略，解决异构机器人团队在行星探索中的目标分配与调度问题，相比传统规划方法具有更好的实时性。


<details>
  <summary>Details</summary>
Motivation: 行星探索需要多机器人团队协作，传统规划算法在问题规模增大时计算复杂度急剧增加，导致规划周期长、推理成本高，难以实现实时规划。

Method: 采用基于学习的多智能体近端策略优化（MAPPO）方法，协调异构机器人团队解决复杂的目标分配和调度问题。

Result: 通过穷举搜索获得的单目标最优解进行基准测试，并在行星探索场景中评估了在线重新规划能力。

Conclusion: 基于学习的方法将扩展性问题从运行时转移到训练时，为实现实时规划迈出了关键一步，MAPPO策略在多机器人协同规划中具有实际应用价值。
...
### [31] [Functional Force-Aware Retargeting from Virtual Human Demos to Soft Robot Policies](https://arxiv.org/abs/2604.01224)
...
TL;DR: SoftAct框架通过显式建模接触力，将人类操作技能迁移到软体机器人手上，相比传统方法显著提升轨迹跟踪精度和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅重定向人类关节轨迹，无法有效处理软体机器人手与人类手之间的极端形态差异和非线性柔顺性，特别是在接触丰富的操作任务中。

Method: 采用两阶段力感知重定向算法：第一阶段将演示的接触力分配到人类手指并比例分配给机器人手指，建立力平衡映射；第二阶段结合末端执行器姿态跟踪和测地加权接触细化进行在线重定向。

Result: 相比运动学和基于学习的基线方法，SoftAct将指尖轨迹跟踪RMSE降低达55%，跟踪方差降低达69%，在零样本真实世界部署和仿真中均获得更高成功率。

Conclusion: 显式建模接触几何和力分布对于软体机器人手的有效技能迁移至关重要，仅靠运动学模仿无法恢复这些信息，SoftAct框架能自然适应极端形态差异和非线性柔顺性。
